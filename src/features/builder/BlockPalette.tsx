...
function DraggableBlock({ def }: { def: BlockDef }) {
  const [{ isDragging }, drag] = useDrag(() => ({
    type: 'template-block',
    item: { type: 'template-block', blockType: def.type },
  ...
  return (
    <div
      ...
      className={cn(
        'flex items-center gap-2.5 p-2.5 rounded-lg border border-border cursor-grab active:cursor-grabbing',
        'hover:bg-background hover:border-accent-dark transition-all duration-100 bg-paper',
    ...
    >
      <span className="text-base w-5 flex-shrink-0 text-center">{def.icon}</span>
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-4">
        {activeCategory === 'All'
          ? Object.entries(grouped).map(([cat, defs]) => (
              <div key={cat}>
                <p className="text-[10px] font-semibold uppercase tracking-widest text-secondary mb-2 px-0.5">
                  {cat}
                </p>
                <div className="space-y-1">
                  {defs.map(def => (
                    <div key={def.type} onDoubleClick={() => onAddBlock(def.type)}>
                      <DraggableBlock def={def} />
                    </div>
                  ))}
                </div>
              </div>
        ...
        {filtered.length === 0 && (
          <p className="text-xs text-secondary text-center py-6">No blocks found</p>
        )}

        <p className="text-[10px] text-secondary text-center py-2">
